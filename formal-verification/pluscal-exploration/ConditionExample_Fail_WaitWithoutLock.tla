------------------------ MODULE ConditionExample_Fail_WaitWithoutLock ------------------------
(*
...
(* --algorithm ConditionExample {
    ...
    \* BUG: Commented out lock acquisition - should fail assertion
    fair process (Consumer \in {2})
    {
      ...
      c_wait_while_empty:
        ...
        \* BUG: Trying to wait without holding lock!
        if (buffer = 0) {
...
c_wait_while_empty(self) == /\ pc[self] = "c_wait_while_empty"
                            ...
                            /\ IF buffer = 0
                                  THEN /\ Assert(pendingCommand = NoCmd, 
                                                 "Failure of assertion at line 138, column 9 of macro called at line 345, column 13.")
                                       /\ Assert(running = self, 
                                                 "Failure of assertion at line 139, column 9 of macro called at line 345, column 13.")
                                       /\ pendingCommand' = [cmd |-> "ConditionWait", task |-> self]
                                       /\ running' = NoTask
                                  ELSE /\ TRUE
...
c_complete(self) == /\ pc[self] = "c_complete"
                    /\ running = self
                    /\ Assert(pendingCommand = NoCmd, 
                              "Failure of assertion at line 152, column 9 of macro called at line 364, column 9.")
                    /\ Assert(running = self, 
                              "Failure of assertion at line 153, column 9 of macro called at line 364, column 9.")
                    /\ pendingCommand' = [cmd |-> "Complete", task |-> self]
                    /\ running' = NoTask
                    /\ pc' = [pc EXCEPT ![self] = "Done"]
                    /\ UNCHANGED << taskState, joinWaiters, lockOwner, 
...
Consumer(self) == c_wait_while_empty(self) \/ c_consume(self)
                     \/ c_mark_done(self) \/ c_complete(self)
