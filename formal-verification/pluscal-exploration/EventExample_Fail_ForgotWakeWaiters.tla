------------------------ MODULE EventExample_Fail_ForgotWakeWaiters ------------------------
(*
 * PURPOSE: NEGATIVE TEST - Forgot to wake waiting tasks
 * EXPECTED: Should violate liveness property (EventuallyWaiterReceives)
 *
      ...
      handle:
        while (TRUE) {
            ...
            if (pendingCommand.cmd = "Spawn") {
                ...
                eventSet := TRUE;
                \* BUG: Forgot to wake waiters! They stay blocked forever.
                taskState := [t \in Tasks |->
                    IF t = pendingCommand.task THEN "Ready"
                    ELSE taskState[t]];
